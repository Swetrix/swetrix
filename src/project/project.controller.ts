import {
  ...
  export class ProjectController {
    ...
    @UseGuards(RolesGuard)
    @Roles(UserType.ADMIN)
    async createForAdmin(@Param('id') userId: string, @Body() projectDTO: ProjectDTO): Promise<Project> {
    ...
    @Roles(UserType.ADMIN)
    @ApiResponse({ status: 200, type: Project })
    async updateForAdmin(@Param('pid') id: string, @Body() projectDTO: ProjectDTO): Promise<any> {
