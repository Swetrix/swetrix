  ...
  return (
    <OutsideClickHandler onOutsideClick={() => setShow(false)}>
      <p className='h-5 mr-2 text-gray-900 dark:text-gray-50 text-xl cursor-pointer' onClick={() => setShow(!show)}>
        {live} {show ? <ChevronUpIcon className='inline w-5 h-5' /> : <ChevronDownIcon className='inline w-5 h-5' />}
      </p>
      {show && (
        <div className='absolute z-10 mt-2 right-0 top-20 text-gray-900 bg-white border border-gray-200 rounded-md shadow-lg dark:bg-slate-900 dark:border-slate-700/50 min-w-[250px] max-h-[200px] overflow-auto'>
            ...
            {loading ? (
              ...
              _map(liveInfo, ({ dv, br, os, cc }, index) => (
                <div
                  ...
                  className='flex flex-row items-center justify-between w-full p-2 mt-2 text-sm text-gray-900 bg-gray-100 rounded-md dark:text-gray-50 dark:bg-slate-800'
                >
                  <div className='flex flex-row items-center'>
                    <Flag className='rounded-sm mr-2' country={cc} size={21} alt='' aria-hidden='true' />
          ...
          <XMarkIcon
            className='absolute rounded-md top-2 right-2 w-5 h-5 text-gray-900 cursor-pointer dark:text-gray-50 hover:bg-gray-200 hover:dark:bg-slate-800 '
