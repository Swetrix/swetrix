...
const formatRangeLabel = (
  ...
  if (mode === 'single' || value.length === 1 || !value[1])
...
const DatePicker = forwardRef<DatePickerHandle, DatePickerProps>(
  function DatePickerInner(
    ...
    const absoluteMinDate = useMemo(
      () => startOfDay(removeMonths(new Date(), maxDateMonths)),
      [maxDateMonths],
    )

    const [open, setOpen] = useState(false)
    const [anchorMonth, setAnchorMonth] = useState<Date>(() =>
      startOfDay(value?.[0] || today),
    )
    const [start, setStart] = useState<Date | null>(() =>
      value?.[0] ? startOfDay(value[0]) : null,
    )
    const [end, setEnd] = useState<Date | null>(() =>
      value?.[1] ? startOfDay(value[1]) : null,
    )
    ...
    useOnClickOutside(
      popoverRef as React.RefObject<HTMLDivElement | HTMLUListElement>,
      () => setOpen(false),
    )
    ...
    const weekdays = useMemo(() => {
      ...
      return Array.from({ length: 7 }).map((_, i) =>
        weekFormatter.format(addDays(base, i)),
      )
    }, [weekFormatter])
    ...
    const rangeBounds = useMemo(() => {
      ...
      return {
        min: min < absoluteMinDate ? absoluteMinDate : min,
    ...
    const setAnchor = (year: number, month: number) => {
      ...
      if (clampedYear === minYear)
        clampedMonth = Math.max(minMonthOfMinYear, clampedMonth)
      if (clampedYear === maxYear)
        clampedMonth = Math.min(maxMonthOfMaxYear, clampedMonth)
        ...
        const clamped = startOfDay(
          new Date(
            _clamp(
              sd.getTime(),
              minAllowed.getTime(),
              Math.min(maxAllowed.getTime(), today.getTime()),
            ),
          ),
        )
    ...
    const inRange = (d: Date) => {
      ...
      const [lo, hi] = end
        ...
        : hoverDate
          ? [
              start <= hoverDate ? start : hoverDate,
              start <= hoverDate ? hoverDate : start,
            ]
          : [null, null]
    ...
    return (
      <div
        className={
          options
            ? undefined
            : cn('relative inline-block h-0 align-top', className)
        }
      >
        ...
        {options ? (
          <input
            ...
            value={label || ''}
            placeholder={
              isSingleMode ? 'Select a date...' : 'Select a date range...'
            }
