...
export class SessionReplayS3Service {
  private getConfig(): S3Config | null {
    ...
    const region =
      process.env.SESSION_REPLAY_S3_REGION ||
      (endpoint ? inferHetznerRegion(endpoint) : '') ||
      'us-east-1'
    const forcePathStyle =
      process.env.SESSION_REPLAY_S3_FORCE_PATH_STYLE === 'true'
    const bucket =
    ...
    for (let attempt = 0; attempt < attempts; attempt++) {
      ...
      try {
        ...
        if (
          ...
          lastError = new Error(
            `S3 ${method} failed with status ${response.status}`,
          )
          continue
        }
  ...
  private async signedFetchOnce(
    ...
    if (!config) {
      throw new Error('S3 session replay storage is not configured')
    }
    ...
    if (config.forcePathStyle) {
      endpoint.pathname = `/${encodeURIComponent(config.bucket)}/${encodeKeyPath(key)}`
    } else {
