...
export class GSCService {
  ...
  /**
   * Whether the project can currently be queried: Search Console has to be
   * configured on this instance, an account connected and a property linked.
   * Unlike getStatus this needs no account email, so it is safe to expose to
   * anyone with project read access.
   */
  async getConnectionState(
    pid: string,
  ): Promise<{ connected: boolean; property: string | null }> {
    if (!this.isAvailable()) {
      return { connected: false, property: null }
    }
