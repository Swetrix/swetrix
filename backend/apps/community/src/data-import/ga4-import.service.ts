...
export class Ga4ImportService {
  ...
  private getCredentials() {
    ...
    if (!clientId || !clientSecret) {
      throw new InternalServerErrorException(
        'Google Analytics import is not configured on this instance. Set GOOGLE_GA4_CLIENT_ID and GOOGLE_GA4_CLIENT_SECRET on the API container.',
      )
    }
  ...
  private getRedirectUrl(): string {
    ...
    if (!base) {
      throw new InternalServerErrorException(
        'BASE_URL is not configured. Set BASE_URL on the API container to enable Google Analytics import.',
      )
    }
    return `${base}/ga4-import-connected`
  }
  ...
  isConfigured(): boolean {
    ...
    return !!(clientId && clientSecret && getBaseUrl())
  }
}
