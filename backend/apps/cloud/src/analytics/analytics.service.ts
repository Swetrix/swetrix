...
export class AnalyticsService {
  ...
  getFiltersQuery(
    ...
    // Append entry/exit page filters only for analytics (not for custom events)
    if (!customEVFilterApplied && _size(entryExitFilters.entryPage)) {
      query += ' AND ('
    ...
    if (!customEVFilterApplied && _size(entryExitFilters.exitPage)) {
      query += ' AND ('
  ...
  */
  ...
  async getFilters(pid: string, type: string): Promise<Array<string>> {
    ...
    if (type === 'ev') {
      query = `SELECT ev FROM customEV WHERE pid={pid:FixedString(12)} AND ev IS NOT NULL GROUP BY ev`
