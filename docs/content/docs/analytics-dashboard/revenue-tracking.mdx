...
<Steps>
  <Step>
    ### Connect a source Open **Project Settings &gt; Revenue** and connect Stripe or Paddle from
    its provider section, or use the API by sending your first transaction. Choose the reporting
    currency Swetrix should use in your analytics (USD, EUR, or GBP).
  </Step>
  <Step>
...
| `currency`      | string                               | yes      | ISO 4217 currency code (e.g. `USD`, `EUR`). Converted to your project's reporting currency automatically.                                  |
...
<Steps>
  <Step>
    ...
    Go to your project dashboard and open **Project Settings &gt; Revenue**. The Stripe section shows whether Stripe is connected and includes the API key field.
  </Step>
  <Step>
    ### Create a restricted API key

    The settings page includes a direct link to create a Stripe restricted key with the correct permissions pre-selected. Click **Create a key in Stripe** and confirm it in Stripe.
  ...
  <Step>
    ...
    Paste the restricted key (starts with `rk_live_`) into the Stripe API key field and click **Connect**. Use **Reporting settings** on the same page to choose the currency Swetrix should use in analytics.
...
<Steps>
  <Step>
    ### Open the Revenue settings
    Go to your project dashboard and open **Project Settings &gt; Revenue**. The Paddle section shows whether Paddle is connected and includes the API key field.
  </Step>
  <Step>
    ...
    - Adjustments, Transactions, Subscriptions, Customers, Products, Prices (read)

  </Step>
  <Step>
    ### Connect

    Paste the API key (starts with `pdl_live_`) into the Paddle API key field and click **Connect**. Use **Reporting settings** on the same page to choose the currency Swetrix should use in analytics.
...
If you run Google Ads, connect the [Google Ads integration](/integrations/google-ads) to combine campaign spend with your revenue data. The [Ads dashboard](/analytics-dashboard/ads) then shows attributed revenue, ROAS, and CPA per campaign.

For this to work, pass the **session ID** with your transactions (`sessionId` on the API, or `swetrix_session_id` in Stripe/Paddle metadata as shown above) — that's the link Swetrix uses to trace a purchase back to the browsing session, and from there to the ad campaign that started it.

## Managing your connection

You can disconnect or change your connected provider at any time from **Project Settings &gt; Revenue**. Disconnecting removes the API key (or disables API ingestion) but keeps previously recorded revenue data in your dashboard.

To change the reporting currency after connecting, use **Reporting settings** on the same settings page. Currency conversion is applied to all transactions (past and future) based on exchange rates at sync time.
