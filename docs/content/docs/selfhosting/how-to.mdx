...
description: "Deploy Swetrix on your own infrastructure with Docker and configure the services needed to run your website analytics instance."
---
...
Download the [self-hosting repository](https://github.com/swetrix/selfhosting) from GitHub.
...
The repository contains a `compose.yaml` file with all the necessary configuration to run Swetrix.
...
Run the `configure.sh` script to set up the environment variables. It will help you configure and autogenerate some necessary variables.
...
The script will save the environment variables to a `.env` file in the repository directory. You can later change them manually if needed. Please refer to the [configuration section](/selfhosting/configuring) for more information.

</Step>
<Step>
### Run the containers

Once all the environment variables are set, start the containers:
...
<Tabs items={["Nginx (default)", "Traefik"]}>
<Tab value="Nginx (default)">
...
If you'd rather not run Docker Compose on a server yourself, the community maintains templates that deploy Swetrix Community Edition on managed hosting platforms.

<Callout type="warn">
  These templates are built and maintained by the community, not by the Swetrix team. We don't
  control their contents or release schedule, so please report any issues with them to their
  maintainers.
</Callout>

### Railway

[Railway](https://railway.com) provisions and runs the containers for you. The community-maintained template deploys the Swetrix frontend, API, Redis and ClickHouse, wires them together and gives you an HTTPS URL:

[![Deploy on Railway](/img/integrations/railway-button.svg)](https://railway.com/deploy/swetrix)

The template's source lives at [osbytes/template-swetrix](https://github.com/osbytes/template-swetrix).

## Session replays

To record and play back sessions, configure your own private S3-compatible bucket using the [Session replay setup guide](/selfhosting/session-replays). Replay storage is optional; ordinary analytics works without it.
