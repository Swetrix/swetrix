...
If you'd rather not manage a server yourself, a community-maintained [Railway](https://railway.com) template provisions the frontend, API, Redis and ClickHouse, wires them together, and gives you an HTTPS URL:

[![Deploy on Railway](https://railway.com/button.svg)](https://railway.com/deploy/swetrix)

The template is built from the same images as the self-hosting repository, and its source is at [osbytes/template-swetrix](https://github.com/osbytes/template-swetrix). The rest of this page covers running Swetrix with Docker Compose on your own server.
