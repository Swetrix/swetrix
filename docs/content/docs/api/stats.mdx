...
slug: /statistics-api
description: "Retrieve website analytics with the Swetrix Statistics API, including request parameters, authentication, filters, and response formats."
---

A newer, per-dimension [Statistics API v2](/statistics-api-v2) is available — it offers human-readable dimension and metric names, server-side pagination and a consistent response format. This v1 API keeps working as documented below.
...
Rate limit for the API depends on your plan. You can find more information on the billing page.
The current rate limits are:

- **Standard plan**: 300 requests per hour;
- **Plus plan**: 6000 requests per hour;
- **Enterprise plan**: custom, and may be unlimited on demand.
...
- **column** - the column to filter by. Possible values are: `cc` (country code), `rg` (region), `ct` (city), `host` (hostname), `pg` (page), `lc` (locale), `br` (browser), `brv` (browser version), `os` (operating system), `osv` (OS version), `dv` (device), `ref` (referrer), `so` (source), `me` (medium), `ca` (campaign), `te` (UTM term), `co` (UTM content), `isp` (ISP), `og` (organisation), `ut` (usage type), `ctp` (connection type).
...
Optional event scope for the returned sessions. Supported values are `traffic` (default), `performance`, and `error`. Use `performance` to return sessions that recorded performance events in the selected time range, or `error` to return sessions affected by errors in the selected time range.
...
When revenue tracking is enabled and transactions are attributed to the session, `details.revenue` contains the session's net revenue after refunds, and `details.refunds` contains the total refunded amount for the session. Values use the project's configured revenue currency.

### GET /v1/log/funnel

Returns a funnel analysis for a specific set of pages or a saved funnel.

```bash
curl 'https://api.swetrix.com/v1/log/funnel?pid=YOUR_PROJECT_ID&pages=["/","/pricing","/signup"]&period=7d'\
  -H "X-Api-Key: ${SWETRIX_API_KEY}"
```
...
A stringified JSON array of [filter objects](#filters). Filters select the session cohort for the funnel analysis.
...
```bash
curl 'https://api.swetrix.com/v1/log/funnel-sessions?pid=YOUR_PROJECT_ID&funnelId=FUNNEL_ID&step=2&period=7d&take=30&skip=0'\
  -H "X-Api-Key: ${SWETRIX_API_KEY}"
```
...
Set to `true` to return sessions that reached the requested step but did not continue to the next step.
...
Returns the most popular journeys — the ordered sequences of pages visitors viewed within a session. This is the data behind the Journeys tab of the dashboard.

```bash
curl 'https://api.swetrix.com/v1/log/journeys?pid=YOUR_PROJECT_ID&period=7d&steps=3&journeys=20'\
  -H "X-Api-Key: ${SWETRIX_API_KEY}"
```

Each journey in the response is a `path` (an array of page paths) together with `value` — the number of sessions that followed it. The response also includes `totalSessions`, the total number of sessions in the period.

#### Parameters

In addition to the parameters of the [`/log` endpoint](#get-v1log):

<hr />

**steps** (optional, default: `3`)

How many pages deep each journey goes (2 - 10).

<hr />

**journeys** (optional, default: `20`)

How many of the most popular journeys to return (5 - 100).

### GET /v1/log/journey-sessions

Returns the sessions that viewed a given page at a given journey step.

```bash
curl 'https://api.swetrix.com/v1/log/journey-sessions?pid=YOUR_PROJECT_ID&period=7d&step=2&page=%2Fpricing'\
  -H "X-Api-Key: ${SWETRIX_API_KEY}"
```
...
The journey step the page was visited at (1-indexed).

<hr />

**page** (required)

The page path visited at the given step.
...
Returns statistics for a specific goal, including conversions, conversion rate, trend, segment breakdowns, and time-to-convert metrics. Simple goals and multi-condition goals are both supported; for multi-condition goals using `AND`, the conversion timestamp is the point where the required conditions have all been satisfied in the session.
...
A stringified JSON array of [filter objects](#filters).
...
Returns paginated sessions that matched a specific goal during the selected time range.

```bash
curl 'https://api.swetrix.com/goal/GOAL_ID/sessions?period=7d&take=30&skip=0'\
  -H "X-Api-Key: ${SWETRIX_API_KEY}"
```
