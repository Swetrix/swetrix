...
slug: /admin-api
description: "Use the Swetrix Admin API to manage projects and automate analytics administration with authenticated requests and documented parameters."
---
...
| `active`              | `boolean`                   | Set to `true` to collect analytics data. Set to `false` to stop collecting new events while keeping existing data and settings available.                                                                                 |
...
| `origins`             | `Array<string>`             | Domains allowed to send analytics using this Project ID. Traffic from all other domains is blocked. Supports wildcards, for example: `['cornell.edu', 'app.example.com', '*.gov.uk']`. An empty array allows all domains. |
| `ipBlacklist`         | `Array<string>`             | IP addresses or CIDR ranges whose traffic should be ignored for this project. Useful for excluding your own visits. For example: `['172.126.10.16', '192.168.0.1/32', '::1']`.                                            |
| `ipWhitelist`         | `Array<string>`             | IP addresses or CIDR ranges that are never flagged by [bot protection](/bot-protection#trusted-ip-addresses). Useful for server-side event tracking. For example: `['172.126.10.16', '192.168.0.1/32', '::1']`.           |
| `botsProtectionLevel` | `off`, `basic`, or `strict` | Choose how aggressively bot and automated traffic should be filtered. `off` tracks all traffic, `basic` filters common bots, and `strict` adds stronger automated-traffic checks. `basic` by default.                     |
...
Project views are the API representation of [Segments](/analytics-dashboard/segments) — saved combinations of dashboard filters and custom event metrics that users can switch between from the dashboard.
...
| Name          | Type      | Required | Description                                                                                                                                                                               |
| ------------- | --------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `column`      | `string`  | `true`   | The traffic column to filter on. One of: `cc`, `rg`, `ct`, `host`, `pg`, `lc`, `br`, `brv`, `os`, `osv`, `dv`, `ref`, `so`, `me`, `ca`, `te`, `co`. Unsupported columns are stripped out. |
| `filter`      | `string`  | `true`   | The value to match against.                                                                                                                                                               |
| `isExclusive` | `boolean` | `true`   | If `true`, rows matching `filter` are excluded; otherwise only matching rows are kept.                                                                                                    |
| `isContains`  | `boolean` | `false`  | If `true`, `filter` is treated as a substring match instead of an exact match.                                                                                                            |
...
| Name              | Type                             | Required | Description                                                         |
| ----------------- | -------------------------------- | -------- | ------------------------------------------------------------------- |
| `customEventName` | `string`                         | `true`   | Name of the [custom event](/events-api) to aggregate (1–100 chars). |
| `metricKey`       | `string`                         | `true`   | Display key for the metric on the dashboard (1–100 chars).          |
| `metaKey`         | `string`                         | `false`  | Optional event metadata key to group on (max 100 chars).            |
| `metaValue`       | `string`                         | `false`  | Optional event metadata value to filter on (max 100 chars).         |
| `metaValueType`   | `string` or `integer` or `float` | `true`   | How to interpret `metaValue` when aggregating.                      |

#### 'Create project view' payload

| Name           | Type                       | Required | Description                                                                                            |
| -------------- | -------------------------- | -------- | ------------------------------------------------------------------------------------------------------ |
| `name`         | `string`                   | `true`   | A display name for the view (1–20 chars).                                                              |
| `type`         | `traffic` or `performance` | `true`   | Which dashboard the view applies to. Cannot be changed after creation.                                 |
| `filters`      | `Array<Filter>`            | `false`  | Filters to pre-apply when the view is selected. See [Filter object](#filter-object).                   |
| `customEvents` | `Array<CustomEvent>`       | `false`  | Custom-event metrics to display with the view. See [Custom event object](#custom-event-object). Max 3. |
...
| Name           | Type                 | Description                                                                                       |
| -------------- | -------------------- | ------------------------------------------------------------------------------------------------- |
| `name`         | `string`             | A display name for the view (1–20 chars).                                                         |
| `filters`      | `Array<Filter>`      | Replaces the stored filters. See [Filter object](#filter-object).                                 |
| `customEvents` | `Array<CustomEvent>` | Replaces the stored custom-event metrics. See [Custom event object](#custom-event-object). Max 3. |
...
List all [Segments](/analytics-dashboard/segments) (project views) for a project. For password-protected projects, pass the dashboard password via the `X-Password` header.
