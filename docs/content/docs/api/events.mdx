...
| `profileId` | `string` | `true`   | A unique, stable identifier of the user (e.g. an internal user ID), max 256 characters. It's hashed together with the project ID server-side; the raw value is never stored. |
...
curl -i -X POST https://api.swetrix.com/log/identify \
  ...
  -H "X-Client-IP-Address: 192.0.2.1" \
  -H "Content-Type: application/json" \
  -d '{"pid":"YOUR_PROJECT_ID","profileId":"user-12345"}'
```

```json title="Response (201 Created)"
{ "profileId": "usr_8214637194021987452" }
```
