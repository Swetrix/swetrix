...
description: "Configure a Nginx reverse proxy for Swetrix to forward tracking requests through your domain and preserve accurate visitor information."
---

If you are already using Nginx on your website, you can add Swetrix to your configuration to proxy it.

<Callout type="warn">
  The `proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;` line in the configuration below
  is **essential — do not remove it**. It forwards your visitor's real IP address to Swetrix.
  Without it, all events arrive from your server's datacenter IP: geolocation data will show your
  server's location instead of your visitors', and if your project's [bot protection
  level](/bot-protection) is set to **Strict**, your traffic will be dropped as datacenter traffic.
</Callout>
