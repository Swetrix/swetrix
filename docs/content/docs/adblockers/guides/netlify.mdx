...
description: "Configure a Netlify reverse proxy for Swetrix to forward tracking requests through your domain and preserve accurate visitor information."
---
...
<Callout type="info">
  Swetrix reads the visitor's IP address from the `X-Forwarded-For` header, which Netlify sets
  automatically when proxying requests — so geolocation, unique visitor counting and [bot
  protection](/bot-protection) keep working as normal.
</Callout>
