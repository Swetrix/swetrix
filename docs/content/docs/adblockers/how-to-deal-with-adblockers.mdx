...
description: "Learn how adblockers affect Swetrix tracking and compare reverse proxy options for serving analytics requests through your own domain."
---
...
| **Visitor IP & geolocation**     | Preserved automatically                                            | Preserved via the `X-Forwarded-For` header (set automatically by Vercel/Netlify; on your own Nginx, forward it as shown in the guide) |
...
<Callout type="info">
  Swetrix reads the visitor's real IP address from the `X-Forwarded-For` header. Hosting platforms
  like Vercel and Netlify set it automatically when proxying; if you run your own web server, make
  sure it forwards the header (the Nginx guide shows how) — otherwise geolocation will reflect your
  server's location and [bot protection](/bot-protection) set to **Strict** will drop the traffic as
  datacenter traffic.
</Callout>
