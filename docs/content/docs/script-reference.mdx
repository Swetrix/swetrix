...
<Callout type="warn">
  ...
  Don't use email addresses (they're personal data and can change) or values like `null`,
  `undefined` or `"guest"` (the API rejects those, as they would fuse unrelated visitors into a
  single profile). The ID you pass is hashed together with your project ID server-side and is never
