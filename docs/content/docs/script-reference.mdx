...
<Callout type="warn">
  ...
  unrelated visitors into a single profile. The ID you pass is stored as provided (behind a `usr_`
  prefix) and shown in your dashboard, so if you want a user's email on their profile, send it as a
  trait rather than using it as the ID.
</Callout>
