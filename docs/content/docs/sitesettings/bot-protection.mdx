...
<Callout type="warn">
  Self-hosted installations do not include the commercial DB-IP ISP database out of the box, so the
  `datacenter_ip` reason will simply not appear in the report. Strict mode still gives you the other
  four checks, which already cover the vast majority of automated traffic.
</Callout>
...
The other four detections (user agent, headless browser, suspicious headers, probe paths, referrer spam) run identically on community and cloud, and the report renders the same way.
