...
slug: /how-to-embed
description: "Embed a Swetrix analytics dashboard on your website and configure public access so visitors can explore your website statistics."
---
...
```html
<iframe
  src="https://swetrix.com/projects/YOUR_PROJECT_ID?tab=traffic&theme=dark&embedded=true"
  width="1200"
  height="700"
  title="Test"
></iframe>
...
- `tab`: `traffic`, `seo`, `performance`, `profiles`, `funnels`, `sessions`, `replays`, `errors`, `goals`, `experiments`, `featureFlags`, `captcha`, `ai` - the default tab to show. By default, the traffic tab is shown.
- `tabs`: `traffic`, `seo`, `performance`, `profiles`, `funnels`, `sessions`, `replays`, `errors`, `goals`, `experiments`, `featureFlags`, `captcha`, `ai` - a comma-separated list of tabs that should be accessible in dashboard. Example: `tabs=traffic,performance,sessions` will only show the traffic, performance and sessions tabs. By default, all tabs are shown.
- `embedded` - if set to `true`, header, footer and any marketing CTAs will be hidden.
- `collapsed`: if set to `true`, the sidebar will be collapsed by default.
