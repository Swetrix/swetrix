...
description: "Integrate Swetrix with Ruby on Rails using this installation guide. Add privacy-friendly website analytics and verify that tracking works."
---
...
<html>
  <head>
    ...
    <%= csrf_meta_tags %> <%= csp_meta_tag %> <%= stylesheet_link_tag "application",
...
```html
<%= form_with url: "/signup", id: "signup-form" do |f| %> <%= f.email_field :email, placeholder:
"you@example.com" %> <%= f.submit "Sign up" %> <% end %>
