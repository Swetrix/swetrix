...
description: "Integrate Swetrix with FastAPI using this installation guide. Add privacy-friendly website analytics and verify that tracking works."
---
...
class SwetrixClient:
    ...
    async def track_pageview(self, ip: str, user_agent: str, **kwargs):
        ...
        await self._client.post(
            self.api_url,
            json=payload,
            headers={"User-Agent": user_agent or "", "X-Forwarded-For": ip, "Content-Type": "application/json"},
        )
