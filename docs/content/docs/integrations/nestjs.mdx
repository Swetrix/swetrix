...
description: "Integrate Swetrix with NestJS using this installation guide. Add privacy-friendly website analytics and verify that tracking works."
---

Integrate Swetrix with your [NestJS](https://nestjs.com/) application using the [`@swetrix/node`](https://github.com/Swetrix/swetrix/tree/main/packages/tracker-node) server-side SDK to track page views, monitor errors, and capture custom events — all while staying privacy-friendly and GDPR-compliant.
...
export class SwetrixService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.swetrix = new Swetrix(this.configService.get<string>("SWETRIX_PROJECT_ID"), {
...
@Module({
  imports: [ConfigModule.forRoot({ isGlobal: true }), SwetrixModule],
})
export class AppModule {}
```

<Callout type="warn">
Replace `YOUR_PROJECT_ID` with your actual Project ID from the [Swetrix dashboard](https://swetrix.com/dashboard), or set the `SWETRIX_PROJECT_ID` environment variable.
...
- [`@swetrix/node` source code](https://github.com/Swetrix/swetrix/tree/main/packages/tracker-node) — full source and documentation for the server-side SDK.
