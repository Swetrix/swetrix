...
description: "Integrate Swetrix with Flask using this installation guide. Add privacy-friendly website analytics and verify that tracking works."
---
...
```html
{# templates/partials/swetrix.html #} {% if config.SWETRIX_PROJECT_ID %}
<script src="https://swetrix.org/swetrix.js" defer></script>
<script>
  document.addEventListener("DOMContentLoaded", function () {
    swetrix.init("{{ config.SWETRIX_PROJECT_ID }}");
