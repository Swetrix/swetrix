...
If you set up CSP on your site, make sure to add Swetrix to the list of allowed domains. The simplest way to do this is `Content-Security-Policy: default-src 'self' *.swetrix.com *.swetrix.org`, but we recommend you to read more about CSP and set it up properly.
</Accordion>
<Accordion title="Testing on localhost">
By default, Swetrix doesn't track any events on localhost to prevent developers from polluting their data with test events. If you want to test your integration on localhost, you can enable the `devMode` option in the [script configuration](/swetrix-js-reference#init).
</Accordion>
<Accordion title="Site caching issues">
If you use caching on your site, make sure to purge the cache after you install Swetrix script, to ensure you're loading the latest version of your site.
</Accordion>
<Accordion title="Allowed domains misconfigured">
If you set up allowed domains in the Project settings, you might have misspelled your domain name or added a `www.` prefix to it while your site does not use it (or vice versa).
</Accordion>
<Accordion title="Using an SPA or similar framework">
You might have set up the script improperly. Generally, we recommend using [Swetrix via NPM](/install-script#install-swetrix-via-npm) for such application.
</Accordion>
</Accordions>
