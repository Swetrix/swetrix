...
<Callout type="warn">
  ...
  dashboards, but cross-dimension breakdowns (e.g. "Firefox visits to /pricing from the US") are
  approximated by sampling each dimension independently per day. Per-day totals (visitors, visits,
  pageviews) and single-dimension breakdowns will match Plausible exactly.
</Callout>
...
- **Cross-dimension joints**: combinations like "Firefox on macOS in GB" are sampled under an independence assumption — per-dimension daily totals match, but joint distributions don't
