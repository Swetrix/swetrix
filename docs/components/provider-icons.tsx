...
import { ArrowsLeftRightIcon, CodeIcon } from "@phosphor-icons/react/dist/ssr";
...
export const HttpRequestIcon = ({ className }: { className?: string }) => (
  <ArrowsLeftRightIcon
    className={className ?? REVENUE_ICON_DEFAULT_CLASSNAME}
    weight="duotone"
    aria-hidden
...
export const NodeIcon = ({ className }: { className?: string }) => {
  const id = useId();
  const clipId = `${id}-node-hex`;
  const mainGradientId = `${id}-node-main`;
  const rightGradientId = `${id}-node-r1`;
  const leftGradientId = `${id}-node-r2`;
