...
export function FeedbackWidget() {
  ...
  return (
    <section className='hidden sm:block mt-8 ring-1 ring-gray-300 dark:ring-slate-800/80 rounded-xl p-6 bg-transparent relative overflow-hidden' aria-label='Docs feedback widget'>
      <div className='absolute right-0 -top-4 w-[140px] h-[140px] opacity-10 dark:opacity-5 pointer-events-none' aria-hidden='true'>
        <img src='https://swetrix.com/logo512.png' alt='' className='w-full h-full object-contain -rotate-10' />
      </div>
      <h2 className='text-2xl font-bold mb-2 text-slate-900 dark:text-white mt-2'>Help us improve Swetrix</h2>
            ...
            <form className='mt-4' onSubmit={onSubmitReason}>
              ...
              >
                {NEGATIVE_REASONS.map((label) => (
                  <label key={label} className='flex items-center gap-3 cursor-pointer text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-colors'>
                    <input
                      type='radio'
      ...
      <div className='mt-6 pt-4 border-t border-gray-200 dark:border-slate-800 flex items-center gap-3 flex-wrap text-sm text-slate-500 dark:text-slate-400 relative z-10'>
        <Link className='font-medium hover:text-slate-900 dark:hover:text-white transition-colors' href='/contribute'>
